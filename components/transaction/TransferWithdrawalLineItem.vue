<template>
  <TransactionTransferLineItem :transfer="transfer" display-date>
          ...
          <div>
            <CommonTimer :future-date="transfer.expectedCompletionTimestamp" @finish="emit('timer-finished')">
              <template #default="{ timer, isTimerFinished }">
                ...
                <template v-else
                  >Est. time: <span class="tabular-nums" data-testId="withdraw-timer">{{ timer }}</span></template
                >
              </template>
            </CommonTimer>
          </div>
...
import type { TransferWithStatus } from "@/composables/zksync/useBridgeWithdrawalStatuses";
